 ...
 *
 */
public class DsUserManagementScreen extends DecalsScreen implements ViewHandler {
   ...
   private void buildGroupNavigation() {
      ...
      GroupsViewBuilder.buildGroupNavigation(PGRP_NAV_CONTAINER,PGRP_SELECTIONS,PGRP_MORE_SELECTIONS_CONTAINER,PGRP_LESS_SELECTIONS_CONTAINER,
   ...
   private void buildGroupsView() {
      DsUtil.hideLabel(PGRP_BUSY);
      buildGroupNavigation();
      if (groupManager.getNumberOfGroups() == 0) {
         DsUtil.hideLabel(PGRP_CURRENT_CONTAINER);
         DsUtil.showLabel(PGRP_NONE);
      }
      else {
         currentGroup = groupManager.getGroupList().get(0);
         buildCurrentGroupView();
         DsUtil.hideLabel(PGRP_NONE);         
         DsUtil.showLabel(PGRP_CURRENT_CONTAINER);
      }
   }
  ...
  //initializes delete user transfer user list filtering
   ...
   protected EventCallback addGroupHandler = new EventCallback() {
      ...
      public void onEvent(Event event) {         
         ...
         DsESBApi.decalsCreateGroup(name, GroupType.PUBLIC_TYPE, new ESBCallback<ESBPacket>() {
            @Override
 ...
 //handle add new collection user
   ...
   private void setUpAddGroupUserModal() {
      ...
      ArrayList<AppUser> selectList = groupManager.removeGroupUsersFromUserList(currentGroup.getGroupId(),umHelper.getUserList());
   ...
   public void display() {
      ...
      PageAssembler.attachHandler(PGRP_MORE_SELECTIONS_LINK,Event.ONCLICK,showAllGroupsNavListener);      
      PageAssembler.attachHandler(PGRP_LESS_SELECTIONS_LINK,Event.ONCLICK,showLessGroupsNavListener);      
