...
public class Decals_ui extends AppEntry implements EntryPoint, ValueChangeHandler<String> {
   ...
   private static final String SOLR_URL_PROP = "solr.url";
   ...
   private void parseDecalsProperties(String[] rawProperties) {      
      if ((rawProperties[0].indexOf(SITE_NAME_PROP) != -1)) {
         for (String prop:rawProperties) {            
            if (prop.indexOf(SOLR_URL_PROP) != -1) DsSession.getInstance().setSolrUrl(parseProperty(prop));
            else if (prop.indexOf(IS_TN_ROOT_PROP) != -1) DsSession.getInstance().setInteractiveSearchThumbnailRootUrl(parseProperty(prop));            
         }
      }            
   }
}
